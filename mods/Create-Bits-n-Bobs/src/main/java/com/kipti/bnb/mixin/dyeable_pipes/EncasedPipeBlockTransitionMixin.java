...
public class EncasedPipeBlockTransitionMixin {
    ...
    private void bnb$cacheColorBeforeUnencasing(final LevelAccessor world, final BlockPos pos, final Operation<Void> original) {
    ...
    private void bnb$loadColorAfterUnencasing(final LevelAccessor world, final BlockPos pos, final Operation<Void> original) {
        original.call(world, pos);
        if (world instanceof final Level level)
    ...
    private void bnb$cacheColorBeforeEncasing(final LevelAccessor world, final BlockPos pos, final Operation<Void> original) {
        if (world instanceof final Level level)
            DyedPipeTransitionHelper.saveCurrentDye(level, pos);
        original.call(world, pos);
    }
    ...
    private void bnb$loadColorAfterEncasing(final LevelAccessor world, final BlockPos pos, final Operation<Void> original) {
        original.call(world, pos);
        if (world instanceof final Level level)
            DyedPipeTransitionHelper.applyPreviousDye(level, pos);
    }
