...
public abstract class CogwheelChainShape {
    ...
    public static class CogwheelChainWholeShape extends CogwheelChainShape {
        ...
        public CogwheelChainWholeShape(final List<Vec3> pointsLocal, final double baseRadius) {
            ...
            this.closedLoop = pointsLocal.size() > 2
                    && pointsLocal.get(0).distanceToSqr(pointsLocal.get(pointsLocal.size() - 1)) > MIN_LENGTH_SQR;
        ...
        public Vec3 intersect(final Vec3 from, final Vec3 to) {
            ...
            if (!bounds.inflate(radius).clip(from, to).isPresent()) return null;
